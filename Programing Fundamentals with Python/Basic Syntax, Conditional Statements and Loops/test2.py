...
print(f"Добре дошли в играта Морски Шах. Вие играете с X, а ботът с O. Ето възможните позиции. След всяко действие на "
      f"бота, трябва да въведете на коя позиция искате да сложите X и да натиснете "
...
def check_winner(keep_score):
    global first_pos
    # провери дали някой бие и върни True или False keep_score показва дали да се променят точките, тей като тази
    # функция се извиква от бота когато проверява какъв може да е следващия ход в такъв случай не искам да променя
    # точките
    player_won = -1
    for j in range(1, 3):
        for i in range(3):
...
def bot_move():
    valid_move = False
    winning_move = False
    # режим АТАКА тук проверява ако има движение, което ще му спечели играта (примерно 2 от 3 кръгчета в един ред или
    # диагонал са сложени и може да сложи третото)
    for i in range(9):
        if field[i] != 0:
            continue
        else:
            ...
            if check_winner(False):
                ...
                field[i] = 0
    # режим ЗАЩИТА ако не е намерил такова движение, проверява ако има движение, което ако НЕ направи, ще може ТИ да
    # спечелиш играта в следващия ход
    if not winning_move:
        for i in range(9):
            if field[i] != 0:
                continue
